-- Shop: spend quiz_points_total on streak freeze, shruti box, profile badge.
...
comment on column public.users.shop_streak_freeze_inventory is
  'Consumable streak freeze tokens (arm one before a missed login day).';
comment on column public.users.shop_streak_freeze_armed is
  'When true, the next login sync may bridge one missed calendar day.';
comment on column public.users.shop_shruti_box_owned is
  'Unlocks full shruti box (tanpura loops) on the Shop page.';
...
begin
  ...
  select u.shop_profile_badge into badge
  from public.users u
  where u.id = uid
  for update;
...
-- Leaderboard: expose profile badge
